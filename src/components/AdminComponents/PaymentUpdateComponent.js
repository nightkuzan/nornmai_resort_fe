...
class PaymentUpdateComponent extends Component {
  ...
  componentDidMount() {
    ...
    fetch(
      "http://localhost:3001/payment-info?bookingid=" + this.state.bookingid,
      requestOptions
    )
            ...
            <div className="card-body">
              <div className="row">
                <span className="bg-text-info-2">
                  BOOKING ID : {this.state.bookingidtxt}
                </span>
              </div>
              <div className="row">
              ...
              {this.state.book !== "" || this.state.book != null ? (
                <div className="row">
                  <div className="col-xl-6 col-lg-12">
                    ...
                    />
                    <span
                      className="bg-text-summary"
                      style={{ width: "fit-content" }}
                    >
                      หลักฐานการโอนเงิน
                    </span>
                    {this.state.book.bkTransfer ? (
                      <img
                        src={this.readImage(this.state.book.bkTransfer)}
                        alt="Transfer"
                        style={{ width: "50%", marginTop: "2%" }}
                      />
                    ) : (
                      "ไม่มีการแนบหลักฐานการโอนเงิน"
                    )}
                  </div>
                  <div className="col-xl-6 col-lg-12">
                    <span
                    ...
                    <form onSubmit={this.update}>
                      ...
                      >
                        <option disabled value={this.state.book.status}>
                          {this.state.book.status}
                        </option>
                        {this.state.book.status === "DEPOSIT PAID" ? (
                          ""
                        ) : (
